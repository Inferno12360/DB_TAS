...
<form action="Teilnehmer_erstellen.php" method="post">
    <div class="teb"> 
    ...
    <?php
        ...
        $stmt = $conn->prepare("SELECT * FROM tbl_teilnehmer");
        $stmt->execute();

        if(!empty($_POST["cng_dozent"])) {
            if ($_SERVER["REQUEST_METHOD"] == "POST") { 
                $fields = array( "Vorname", "Nachname","Anrede","EMail", "Telefon", "Hausnummer", "Strasse","Rechnungsstellung", "PLZ", "Ort", "Land");
                foreach ($fields as $key) {
                    updateDozent($conn, "tbl_teilnehmer", $key, $_POST[$key], $_POST["cng_dozent"]);
                }
            }
        }
        ...
        function updateDozent($conn, $table, $target, $value, $primarykey) {
            if (isset($_POST[$target])) { 
                $update = $conn->prepare("UPDATE $table SET $target=:value WHERE PK_Teilnehmer=:cng_dozent");
 ...
 <div><br>
            ...
            <tr>
                ...
                <?php 
                   ...
                   $stmt = $conn->query("SELECT PK_Teilnehmer, Vorname, Nachname, Anrede, EMail, Telefon, Hausnummer, Strasse, Rechnungsstellung, PLZ, Ort, Land FROM tbl_teilnehmer");
                    ...
                    while($row = $stmt->fetch()) {
                        ...
                        <form action="<?= $_SERVER['PHP_SELF']?>" method="POST">
                            <td> <input value="<?php echo $row["PK_Teilnehmer"]?>" readonly name="PK_Teilnehmer" type="text" class="edit"></td>
                            ...
                            <td> <input value="<?php echo $row["Telefon"]?>" name="Telefon" type="text" class="edit"></td>
                            ...
                            <td> <input value="<?php echo $row["Rechnungsstellung"]?>" name="Rechnungsstellung" type="text" class="edit"></td>
                            ...
                            <td>  
                            <form action="<?= $_SERVER['PHP_SELF']?>" method="POST">
                                <button type="submit" name="cng_dozent" value="<?php echo $row["PK_Teilnehmer"]?>"></button>
                                </form>
                            </td>
                            <td>  
                            <form method='post' action='delete_Teilnehmer.php'>
                                <button type="submit" name="PK_Teilnehmer" value="<?php echo $row["PK_Teilnehmer"]?>"></button>
                                </form>
                            </td>
                        </tr>
                        <?php
